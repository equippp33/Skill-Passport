...
export async function scoreAndMaybeFollowUp(args: {
  ...
  const evaluation = await requestStructured({
    ...
    input: [
      ...
      `  questionTranslation.`,
      `- If the answer was thin, vague, evasive, already complete, OR the`,
      `  candidate did not really answer — they skipped, said they don't know,`,
      `  said nothing usable, or went off-topic — set nextQuestion to null.`,
      `  Never invent a follow-up just to have one, and never follow up on a`,
      `  non-answer with "is there anything you'd like to add?".`,
