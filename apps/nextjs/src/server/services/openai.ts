...
export async function scoreAndMaybeFollowUp(args: {
  ...
  const evaluation = await requestStructured({
    ...
    input: [
      ...
      "",
      `Score this answer for ${currentSkill.label} only, then ask ONE follow-up`,
      `on the SAME skill. Put it in nextQuestion (written in`,
      `${ctx.language.promptName}, English rendering in questionTranslation), or`,
      `null in the cases below. There is at most ONE follow-up per skill, so make`,
      `it count — a specific, natural next question about what they SAID:`,
      `- PROBE a borderline answer before it is scored low: if the answer is a`,
      `  genuine attempt but thin, general or purely hypothetical, the follow-up`,
      `  gives them a fair chance to show the skill — invite a concrete example`,
      `  ("can you tell me about a time…") or the next step ("what would you do`,
      `  first…").`,
      `- For a stronger answer, pull the most interesting thread they opened.`,
      ...
      `  thorough there is genuinely nothing worth asking.`,
      `- Never a hollow "is there anything you'd like to add?" — always a real,`,
      `  specific question.`,
