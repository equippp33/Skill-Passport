...
export async function scoreAndMaybeFollowUp(args: {
  ...
  const evaluation = await requestStructured({
    ...
    input: [
      ...
      `follow-up on the SAME skill would help. Put it in nextQuestion (written in`,
      ...
      `  but thin, general or purely hypothetical, ask one gentle follow-up that`,
      `  gives them a fair chance to show the skill — invite a specific example`,
      `  ("can you tell me about a time…") or the next concrete step ("what would`,
      `  you do first…"). Refer to something they actually said.`,
      `- Also ask when a strong answer has an obvious thread worth exploring.`,
      `- Do NOT ask when the answer is already strong and complete, OR when it is`,
      `  empty, off-topic, or a refusal / "I don't know" — those just move on.`,
      `  Never follow up with a hollow "is there anything you'd like to add?".`,
