  ...
  return (
    <dialog
      ...
      className={cn(
        ...
        "m-auto w-[min(42rem,calc(100vw-2rem))] rounded-2xl p-0",
        "border border-border-subtle bg-surface text-content",
        "shadow-[var(--shadow-raised)]",
        "backdrop:bg-content/40 backdrop:backdrop-blur-[2px]",
      )}
    >
      {/* The inner wrapper is what the backdrop check above tests against,
          and what caps the height so only the body scrolls. */}
      <div className="flex max-h-[min(44rem,calc(100vh-4rem))] flex-col">
        <div className="flex items-start justify-between gap-3 border-b border-border-subtle px-5 py-4">
          ...
          <button
            ...
            aria-label="Close"
            className={cn(
              "-mt-1 -mr-1 grid size-8 shrink-0 cursor-pointer place-items-center rounded-lg",
