...
import { UNDERSTANDING_CHECK_BY_KEY } from "~/config/greeting";
...
/**
 * The "did you understand the question?" check, played after a longer silence
 * (before the visible skip countdown). Fixed per language, generated once and
 * cached in R2 forever, shared across every attempt — same pattern as the
 * nudge and the fillers.
 */
export async function GET(
  ...
  if (!bytes) {
    const speech = await generateSpeech(
      UNDERSTANDING_CHECK_BY_KEY[lang as InterviewLanguageKey],
